...
use crate::{interrupt_handler::{InterruptHandler, Interrupt}, game_app::{GAME_SCREEN_WIDTH, GAME_SCREEN_HEIGHT}};
...
self.oam_ram[address as usize] = byte;
	...
	for pixel_x in 0..GAME_SCREEN_WIDTH as u8 {
		...
		// signed addressing
		let mut address = 0x9000u16 + tile_id as u16;
		if address > 0x97FF {
	    ...
	    let tiledata_least_significant_bits = self.vram[tile_start_address + (tilemap_pixel_y as usize % 8) * 2];
	    let tiledata_most_significant_bits = self.vram[tile_start_address + (tilemap_pixel_y as usize % 8) * 2 + 1];

	    // Compute the color id of the given pixel
	    let x_offset_to_pixel: u8 = tilemap_pixel_x % 8;
	    let color_index = ((tiledata_most_significant_bits >> (6 - x_offset_to_pixel)) & 2) | ((tiledata_least_significant_bits >> (7 - x_offset_to_pixel)) & 1);
	    // let color_index = 3;

	    // Paint the current pixel onto the current framebuffer
	    let buffer_index = pixel_x as usize + self.ly as usize * GAME_SCREEN_WIDTH;
	    self.current_framebuffer[buffer_index] = get_background_color_by_index(color_index, self.bgp);
	}
