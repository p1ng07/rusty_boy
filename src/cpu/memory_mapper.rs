...
impl MemoryMapper {
    pub fn fetch_byte(&self, address: usize, cpu_state: &CpuState) -> Option<u8>{
	if address < 0x4000 && *cpu_state == CpuState::Boot {
	    self.boot_rom.get(address).copied()
	} else if address < 0x4000 {
	    self.rom0.get(address).copied()
	}else{
	    None
	}
    }
